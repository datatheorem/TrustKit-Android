 ...
 *
 * <p>It should be used to initialize the App's SSL pinning policy and to retrieve the corresponding
 * {@code SSLSocketFactory} and {@code X509TrustManager}, to be used to add SSL pinning validation
 * to the App's network connections.
 *
 * <p>TrustKit works by extending the <a
 * href="https://developer.android.com/training/articles/security-config.html" target="_blank">
 * Android N Network Security Configuration</a> in two ways:
 *
 * <ul>
 *   <li>It provides support for the SSL pinning functionality of the Android N Network Security
 *       Configuration to earlier versions of Android, down to API level 17. This allows Apps
 *       supporting versions of Android that earlier than N to implement SSL pinning in a way that
 *       is future-proof.
 *   <li>It adds the ability to send reports when pinning validation failed for a specific
 *       connection. Reports have a format that is similar to the report-uri feature of <a
 *       href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Public_Key_Pinning"
 *       target="_blank">HTTP Public Key Pinning</a> and <a
 *       href="https://github.com/datatheorem/trustkit" target="_blank">TrustKit iOS</a>.
 * </ul>
 *
 * For better compatibility, TrustKit will also run on API levels 15 and 16 but its functionality
 * will be disabled.
 *
 * <h3>Supported Android N Network Security Settings</h3>
 *
 * <p>On devices before Android N, TrustKit supports the following XML tags defined in the <a
 * href="https://developer.android.com/training/articles/security-config.html#CertificatePinning"
 * target="_blank"> Android N Network Security Configuration</a> for deploying SSL pinning:
 *
 * <ul>
 *   <li>{@code <domain-config>}.
 *   <li>{@code <domain>} and the {@code includeSubdomains} attribute.
 *   <li>{@code <pin-set>} and the {@code expiration} attribute.
 *   <li>{@code <pin>} and the {@code digest} attribute.
 *   <li>{@code <debug-overrides>}.
 *   <li>{@code <trust-anchors>}, but only within a {@code <debug-overrides>} tag. Hence, custom
 *       trust anchors for specific domains cannot be set.
 *   <li>{@code <certificates>} and the {@code overridePins} and {@code src} attributes. Only raw
 *       certificate files are supported for the {@code src} attribute ({@code user} and {@code
 *       system} values will be ignored).
 * </ul>
 *
 * <p>On Android N devices, the OS' implementation is used and all XML tags are supported.
 *
 * <h3>Additional TrustKit Settings</h3>
 *
 * <p>TrustKit provides additional functionality to not enforce pinning validation and to allow
 * reports to be sent by the App whenever a pinning validation failure occurred.
 *
 * <h4>{@code <trustkit-config>}</h4>
 *
 * <p>The main tag for specifying additional TrustKit settings, to be defined within a {@code
 * <domain-config>} entry. It supports the following attributes:
 *
 * <ul>
 *   <li>{@code enforcePinning}: if set to {@code false}, TrustKit will not block SSL connections
 *       that caused a pinning validation error; default value is {@code false}. When a pinning
 *       failure occurs, pin failure reports will always be sent to the configured report URIs
 *       regardless of the value of {@code enforcePinning}. This behavior allows deploying pinning
 *       validation without the risk of locking out users due to a misconfiguration, while still
 *       receiving reports in order to assess how many users would be affected by pinning.
 *   <li>{@code disableDefaultReportUri}: if set to {@code true}, the default report URL for sending
 *       pin failure reports will be disabled; default value is {@code false}. By default, pin
 *       failure reports are sent to a report server hosted by Data Theorem, for detecting potential
 *       CA compromises and man-in-the-middle attacks, as well as providing a free dashboard for
 *       developers; email <a href="mailto:info@datatheorem.com">info@datatheorem.com</a> if you'd
 *       like a dashboard for your App. Only pin failure reports are sent, which contain the App's
 *       package name, a randomly-generated ID, and the server's hostname and certificate chain that
 *       failed validation.
 * </ul>
 *
 * <h4>{@code <report-uri>}</h4>
 *
 * A URL to which pin validation failures should be reported, to be defined within a {@code
 * <trustkit-config>} tag. The format of the reports is similar to the one described in <a
 * href="https://tools.ietf.org/html/rfc7469#section-2.1.4" target="_blank">RFC 7469 for the HPKP
 * specification</a>. A sample TrustKit report is available <a
 * href="https://github.com/datatheorem/TrustKit-Android/blob/master/docs/sample_report.json"
 * target="_blank"> in the project's repository </a>.
 *
 * <h3>Sample TrustKit Configuration</h3>
 *
 * <p>The following configuration will pin the www.datatheorem.com domain without enforcing pinning
 * validation, and will send pinning failure reports to report.datatheorem.com. It also defines a
 * debug overrides to add a debug certificate authority to the App's trust store for easier
...
public class TrustKit {
     ...
     * target="_blank"> added to the App's manifest</a>.
     *
    ...
    /**
     * Initialize TrustKit with the Network Security Configuration file with the specified resource
     * ID. The Network Security Configuration file must also have been <a
     * href="https://developer.android.com/training/articles/security-config.html#manifest"
     * target="_blank"> added to the App's manifest</a>.
     *
     * @param context the application's context.
    ...
    public static synchronized TrustKit initializeWithNetworkSecurityConfiguration(
        ...
        try {
            ...
            throw new ConfigurationException(
                    "Could not find the debug certificate in the "
     ...
     * connections.
     *
     * <p>The {@code SSLSocketFactory} is configured for the supplied serverHostname, and will
     ...
     *
     * <p>The {@code X509TrustManager} is configured for the supplied serverHostname, and will
     * enforce this domain's pinning policy even if a redirection to a different domain occurs
     * during the connection. Hence validation will always fail in the case of a redirection to a
     * different domain. However, pinning validation is only meant to be used on the App's API
     * server(s), and redirections to other domains should not happen in this scenario.
